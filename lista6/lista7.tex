...
\item A maioria dos algoritmos em grafos que recebem uma representação por matriz de adjacência como entrada requerem tempo $O(n^2)$, mas existem exceções. Determine se um grafo direcionado G contém um sorvedouro universal, isto é, um vértice com grau de entrada n-1 e grau de saída 0, no tempo O(n) dada uma matriz de adjacência de G. 
