...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4cm, scale=1.0, swap]
    % Draw a 7,11 network
    % First we draw the vertices
    \foreach \pos/\name in {{(0,1)/1}, {(2,2)/2}, {(4,2)/3},
                            {(2,0)/4}, {(4,0)/5}, {(6,1)/6}}
        \node[vertex] (\name) at \pos {$\name$};


    \path (1) edge[blue] node [above, black] {11} (2);
    \path (2) edge[blue] node [above, black] {12} (3);
    \path (3) edge[blue] node [above,black] {19} (6);
    \path (1) edge[blue] node [black]{12} (4);
    \path (4) edge[blue] node [black]{11} (5);
    \path (4) edge[blue] node [black]{1} (2);
    \path (5) edge[blue] node [black]{4} (6);
    \path (5) edge[blue] node [black]{7} (3);
...
\begin{frame}{Redução do Problema de Circulação para o Problema do Fluxo Máximo}

\begin{figure}[!h]
\centering
\includegraphics[scale=0.5]{reducao.png}
\caption{Redução do Problema de Circulação para o Problema do Fluxo Máximo}
\end{figure}
\begin{itemize}
...
\begin{frame}{Projeto de Entrevista}

\begin{itemize}
\item Empresa vende $k$ produtos
\item Empresa possui um banco de dados do histórico de compras de seus clientes.
\item Empresa quer enviar uma pesquisa customizada para seus $n$ clientes para entender suas preferências.
\item A pesquisa deve satisfazer as seguintes restrições:

\begin{itemize}
\item Cada cliente recebe perguntas sobre um subconjunto de produtos.
\item Um cliente recebe perguntas apenas sobre os produtos que ele comprou.
\item A pesquisa deve ser informativa mas não tão longa: Cada cliente $i$ deve responder perguntas sobre um número de produtos entre $c_i$ and $c_i'$.
\item Para cada produto devemos coletar informações suficientes: entre $p_j$ e $p_j'$ clientes devem ser responder perguntas sobre o produto $j$.
\end{itemize}
\item É possível projetar uma pesquisa que satisfaça essas restrições?
\end{itemize}
\end{frame}
