...
\item Muitos problemas podem ser formulados e resolvidos em termos de grafos:
\begin{itemize}
\item Caminho mínimo
...
\begin{itemize}
\item Checar se dois vértices estão conectados: O(1).
\item Listar todos os vizinhos de um vértice $v$: O(n)
\item Complexidade de memória: $O(n^2)$
\item Mais adequado para grafos densos, ou seja, $E \approx \frac{V(V-1)}{2}$
\end{itemize}
\end{frame}

\begin{frame}{Lista de Adjacência}
\begin{itemize}
\item Checar se dois vértices estão diretamente conectado: O(d(v))
\item Lista a vizinhança de um vértice: O(d(v))
\item Complexidade de memória O(n+m) 
\item Mais adequado para grafos pouco densos.
\end{itemize}
\end{frame}

\begin{frame}{Implementando Lista de Adjacência}

\begin{itemize}
\item Solução 1: Usar Listas encadeadas

\begin{itemize}
\item Muito esforço de memória e tempo.
\item Usar ponteiros é ruim
\end{itemize}

\item Solução 2: Usar um vetor de vector

\begin{itemize}
\item Fácil de usar e sem problemas de alocação de memória
\item Acesso mais lento
\end{itemize}

\item Solução 3: Usando vetores
\begin{itemize}
\item Assumindo que o número total de arestas é conhecido
\item Rápido e eficiente em memória.
\end{itemize}
...
\begin{frame}[fragile]{Usando vetores}
\begin{itemize}
\item Um vetor de arestas $E$ de tamanho $m$ e um vetor $LE$ de tamanho $n$.
\begin{itemize}
\item E[k].to indica a extremidade final da aresta $k$.
\item E[k].nextID indica a posição da próxima aresta com extremidade inicial igual da aresta $k$
\end{itemize}
...
\item LE[u] = k
\end{itemize}
\item Iterar sobre todas as arestas começada por $u$:
\Fontvi{
...
\begin{frame}{Lista de Adjacência usando vetores}
\begin{figure}[!h]
\centering
\includegraphics[scale=0.6]{listaadjacencia.png}
...
\subsection{Centro de um grafo}
\begin{frame}{Centro de um grafo}

\begin{enumerate}
\item[3] A excentricidade de um vértice $v$, denotado por $E(v)$, é a distância mínima de $v$ até o vértice mais longe de $v$. O centro de um grafo são os vértices que tem o menor valor de excentricidade. Encontre o centro de um grafo $G$.
\end{enumerate}
...
\begin{enumerate}
\item[1] Forneça um algoritmo de tempo linear que tome como entrada um grafo acíclico orientado $G=(V,E)$ e dois vértice $s$ e $t$, e devolva o número de caminhos de $s$ para $t$ em $G$. Por exemplo, existem exatamente quatro caminhos do vértice $p$ para o vértice $v$: $pov$, $poryu$, $posryv$ e $psryu$. (Seu algoritmo só precisa contar os caminhos, não listá-los.)
...
\begin{enumerate}
\item[2] Um percurso de Euler de um grafo não direcionado conectado de Euler $G=(V,E)$ é um ciclo que percorre cada aresta de $G$ exatamente uma vez, embora possa alcançar um vértice mais de uma vez.
\begin{enumerate}
