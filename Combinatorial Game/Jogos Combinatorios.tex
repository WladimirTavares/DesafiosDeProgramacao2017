...
\begin{frame}{Jogo Combinatório}

\BIT
\item 2 jogadores que jogam alternadamente e não podem passar suas vez.
\item Informações perfeitas.
\item Sem influência da sorte.
\item Número finito de movimentos, sem empate, sempre vencedor;
\item Vencedor determinado de acordo com a última jogada (sem pontuação)
\EIT
...
\BIT
\item No jogo normal, o jogador que não pode se mover perde. 
\item No jogo miseré, o jogador que faz o último movimento possível perde.
\item O jogo é  imparcial quando as regras são as mesmas para ambos os jogadores, caso contrário, nós
Chame isso de parcial.
\EIT
\end{frame}
...
\BIT
\item Fibonacci nim é jogado por dois jogadores, que alternam a remoção de moedas  de uma pilha de moedas. 
\item No primeiro movimento, não é permitido tirar todas as moedas, e em cada movimento subseqüente, o número de moedas removidas dever ser um número de Fibonacci. 
\item De acordo com a convenção de jogo normal, o jogador que leva a última moeda ganha. 
\EIT

\begin{enumerate}
\item Faça uma tabela de programação dinâmica para os casos menores.
\item O que acontece se a quantidade de moedas inicial é um número de Fibonacci?
\item O que acontece se a quantidade de moedas inicial não é um número de Fibonacci?
\end{enumerate}
...
\begin{frame}{Take-and-Break Games}
Suponha que cada jogador, por sua vez, seja permitido:

\BIT
\item remover qualquer número de fichas de uma pilha como no nim, ou 
\item dividir uma pilha contendo pelo menos duas fichas em duas pilhas não vazias (não são removidas as fichas).
\EIT

\begin{enumerate}
\item Calcule o número grundy para $n \leq 12$
\item Considerando 3 pilhas com 2, 5 e 7 moedas. Qual jogador tem a estratégia vencedora?
\end{enumerate}
