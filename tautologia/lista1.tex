...
\item $((P \wedge Q) \wedge (P \rightarrow R) \wedge (\neg Q \vee S)) \rightarrow (R \wedge S)$
\item $((A \vee B) \wedge (C \vee \neg B)) \rightarrow (A \vee C)$
\end{enumerate}

Responda as seguintes perguntas utilizando o seu conhecimento atual sobre algoritmos clássicos e suas complexidades.

\begin{enumerate}

\item Existem $n$ páginas na internet $(1 \leq n \leq 10^7)$. Cada página
$i$ tem um page rank diferente $r_i$. Encontre as 10 páginas
com os maiores page rank. Qual método é mais recomendada? Justifique sua resposta.
...
\item Dado um vetor estático $L$ com 10K inteiros, você precisa frequentemente obter $sum(i,j) = L[i] + L[i+1] + \ldots + L[j]$. Qual é a estrutura de dados mais recomendada? Justifique sua resposta.

\begin{enumerate}
\item Para cada consulta, utilize um vetor simples para calcular $L[i] + L[i+1] + \ldots + L[j]$ em $O(N)$.
\item Para cada consulta, utilize um vetor pré-processado usando \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ em $O(1)$.
\end{enumerate}
...
\begin{enumerate}
\item Tente todas as possíveis sub-matrizes e cheque se a média de cada submatriz é 7. Este algoritmo roda em $O(M^3 \times N^3)$
\item Tente todas as possíveis sub-matrizes, mas em $O(M^2 \times N^2)$ usando \_\_\_\_\_\_\_\_\_\_\_.
\end{enumerate}

\item Qual algoritmo produz uma lista dos primeiros 10K números primos com a melhor complexidade de tempo?

\item Você quer saber quantos zeros tem no final de $100!$. O que você deve fazer?

\item Você quer testar se o fatorial de $n$ é divisível por $m$, onde $1 \leq n \leq 10000$. O que você deve fazer?

\item Você quer enumerar todas as ocorrências de uma substring $P$ (de tamanho $m$) em uma string $T$ (de tamanho $n$). Ambos $n$ e $m$ tem no máximo 1M de caracteres. O que você deve fazer?

\item Dado um conjunto de S de N pontos aleatoriamente espalhados em um plano 2D ($2 \leq N \leq 1000000$), encontre dois pontos em S com a maior distância euclidiana entre eles. O que você deve fazer?
